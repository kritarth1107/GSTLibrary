...
import in.co.gstsamadhan.gstsamadhan.Adapter.ExpertAdapter;
...
public class ExpertsActivity extends AppCompatActivity {
    ...
    private void retrieveValues() {
        if(!isConnected(ExpertsActivity.this)){
            ...
            Call<List<Experts>> call = gstSamadhanApi.getExperts();

            call.enqueue(new Callback<List<Experts>>() {
                @Override
                public void onResponse(Call<List<Experts>> call, Response<List<Experts>> response) {
                    List<Experts> retrievedList = response.body();
                    ExpertAdapter expertAdapter = new ExpertAdapter(getApplicationContext(),retrievedList) ;
